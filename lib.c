...
// extern sfRenderWindow* renderWindow_create(sfVideoMode mode, const char* title, sfUint32 style, const sfContextSettings* settings){
extern sfRenderWindow* renderWindow_create(const char* title){
  sfVideoMode videoMode = {400, 300, 32};

  return sfRenderWindow_create(videoMode, "CTGUI example (CSFML-GRAPHICS)", 7, NULL);
}

void func(void) { printf("Button clicked\n"); }

extern int mains(sfVideoMode mode, const char* title, unsigned short style) {
  sfRenderWindow* window = sfRenderWindow_create(
      mode, title, style, NULL);
